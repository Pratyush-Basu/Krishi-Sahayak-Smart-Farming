...
async function fetchCitySuggestions(q) {
  try {
    const res  = await fetch("/irrigation/geo_search?q=" + encodeURIComponent(q));
...
function requestGPS(btn) {
  ...
  navigator.geolocation.getCurrentPosition(
    pos => {
      ...
      fetch(`/irrigation/geo_reverse?lat=${lat}&lon=${lon}`)
        .then(r => r.json())
        .then(d => {
...
function setMode(mode) {
  currentMode = mode;
  fetch("/irrigation/set_mode", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({mode}) });
...
function setPump(state) {
  fetch("/irrigation/set_pump", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({pump:state}) });
...
async function fetchData() {
  try {
    const res = await fetch("/irrigation/data");
